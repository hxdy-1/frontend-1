...
const removeAllAnimations = function () {
    for (let i = 0; i < popupList.length; i++) {
        popupList[i].classList.remove("shakeAnimate", "translateAbove", "translateBelow")
    }
}
...
const handleGameWin = () => {
    ...
    document.getElementById(`player--${playerActive === 0 ? 1 : 0}`).classList.add("loser")
...
shooterEl.addEventListener("click", function () {
    ...
    if (invalidScores.includes(randomlyGeneratedNumber)) {
        ...
        document.getElementById(`current_score--${playerActive}`).textContent = activePlayerCurrentScore;

        addAnimation(randomlyGeneratedNumber, playerActive === 0 ? "translateAbove" : "translateBelow")
    }
})
...
const fetchOverallScore = function () {
    ...
    document.getElementById(`overall_score--${playerActive}`).textContent = overallScores[playerActive]
